...
[![python](https://img.shields.io/badge/python-3.11%20|%203.14-blue)](https://www.python.org/downloads/release/python-3110/)
...
GambitLens is composed of five distinct agents that operate as a unified pipeline:

| Component | Technology | Description |
| :--- | :--- | :--- |
| **Data Ingestion** | Lichess API, Python | Pulls raw game telemetry via REST API (`src/ingestion.py`). |
| **Feature Engineering** | NumPy, Pandas | Calculates centipawn loss, game phase tags, and move durations (`src/features.py`). |
| **Model Training** | Scikit-learn | Trains automated blunder classification models (`src/model.py`). |
| **Diagnostic Visuals** | Matplotlib, SVG API | Renders pure-SVG evaluation charts and clock telemetry (`src/visuals.py`). |
| **Tactical Agent** | Groq Llama3 API | Generates LLM-powered natural language tactical explanations (`src/agent.py`). |
...
All engine components are covered by granular unit tests verified across Python environments:
...
GambitLens executes automatically on a daily schedule via GitHub Actions. Every 24 hours, the runner ingests fresh game telemetry, validates model accuracy, updates diagnostic SVG assets, and synchronizes outputs back to the repository.
